...
#include "Lisch.cpp"
#include "ListFormats/base.h"
#include "ListFormats/list.h"
#include "ListFormats/table.h"
#include "List.types.h"
#include "../vconio2.h"

using namespace std;
using namespace vconio2;
...
int main(int argc, char** argv) {
  ...
  ifstream configFile (string(cCurrentPath) + "\\.config.ini");
  string path = "";
  string line;

  Buffer buffer = {
    ScreenBufferInfos.dwSize,
      ...
      if (line[0] == '[') {
        ...
        if (infos.value != "") {
          if (path == "lisch.buffer") {
            ...
            if (infos.key == "width") buffer.size.X = value;
            else if (infos.key == "height") buffer.size.Y = value;
            else if (infos.key == "x") buffer.pos.X = value;
            else if (infos.key == "y") buffer.pos.Y = value;
          }
  ...
  lst.setBuffer(buffer.size, buffer.pos);
